...
class MessageSerializer(serializers.ModelSerializer):
    class Meta:
        model = Message
        fields = [
            "id", "patient", "channel", "direction", "template", "body", "status",
...
class ThreadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Thread
        fields = [
            "id", "patient", "channel", "owner", "status",
    ...
    def get_unread_count(self, obj) -> int:
        return Message.objects.filter(
            patient=obj.patient, channel=obj.channel, direction=Message.Direction.INBOUND, is_read=False
        ).count()
